...
import type { Exercise, DailyLog, UserProfile, DifficultyLevel, IntensityConfig } from '@/lib/types';
import { WORKOUTS, MAIN_WORKOUT_ID, XP_PER_EXERCISE, XP_PER_WORKOUT_COMPLETION_BONUS, DEFAULT_USER_PROFILE_ID, getXpToNextLevel, CHARACTERS, XP_DIFFICULTY_MULTIPLIERS, XP_EXTRA_QUEST_BONUS } from '@/lib/constants';
...
export function WorkoutLogList() {
  ...
  const [extraQuestActive, setExtraQuestActive] = useState<boolean>(false);
  ...
  const handleFinishWorkout = () => {
    ...
    if (lastWorkoutString) {
        const lastWorkoutDate = new Date(lastWorkoutString + "T00:00:00");
  ...
  const handleExtraQuest = () => {
    ...
    const updatedProfile: UserProfile = {
      ...
      experiencePoints: newExperiencePoints,
      level: newLevel,
      rewards: levelledUp ? [...userProfile.rewards, `Level ${newLevel} (Extra Quest Bonus!)`] : userProfile.rewards,
    };
    setUserProfile(updatedProfile);
    ...
    toast({
      ...
      description: `You've pushed beyond your limits and earned an extra ${bonusXp.toFixed(0)} XP!`,
      duration: 5000,
    });
     if (levelledUp) {
        toast({
      ...
      <Card className="shadow-xl bg-card/80 backdrop-blur-sm">
        ...
        <CardContent className="space-y-6">
          ...
          {!workoutActive && !currentLog.workoutCompleted && (
            ...
            >
              <Zap className="mr-2 h-6 w-6" /> LAUNCH TRAINING SESSION!
            </Button>
          )}
          ...
          {(workoutActive || currentLog.workoutCompleted) && (
            <>
              <div>
                <Label className="text-sm font-medium text-foreground/80">Progress: {currentLog.completedExerciseIds.length} / {workout.exercises.length} exercises ({currentDifficultyButtonConfig[currentDifficulty].text})</Label>
        ...
        <CardFooter className="flex flex-col gap-4">
          {!currentLog.workoutCompleted && workoutActive && (
            <Button 
              ...
              disabled={!userProfile || currentLog.completedExerciseIds.length === 0} 
              size="lg" 
